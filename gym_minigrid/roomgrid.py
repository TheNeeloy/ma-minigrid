...
def ma_reject_next_to(env, pos):
    """
    Function to filter out object positions that are right next to
    any agent's starting point
    """

    out = False
    for agent_pos in env.agent_poses:
        sx, sy = agent_pos
        x, y = pos
        d = abs(sx - x) + abs(sy - y)
        if d < 2:
...
class MARoomGrid(MultiAgentMiniGridEnv):
    """
    Environment with multiple agents, rooms, and random objects.
    This is meant to serve as a base class for other environments.
    """
    ...
    def _gen_grid(self, width, height):
        """
        Generates grid for environment. Make sure to set agent positions in child class.
        """
    ...
    def place_agent(self, i=None, j=None, rand_dir=True):
        ...
        while True:
            super().place_agent(room.top, room.size, rand_dir, max_tries=1000)
            front_cell = self.grid.get(*self.front_pos(len(self.agent_poses)-1))
            if front_cell is None or front_cell.type is 'wall':
                break

        return self.agent_poses[len(self.agent_poses)-1]

    def connect_all(self, door_colors=COLOR_NAMES, max_itrs=5000):
        """
        Make sure that all rooms are reachable by all agents from their
        starting positions
        """
        ...
        for agent_pos in self.agent_poses:

            start_room = self.room_from_pos(*agent_pos)
            added_doors = []
            num_itrs = 0
    ...
    def add_distractors(self, i=None, j=None, num_distractors=10, all_unique=True):
        """
        Add random objects that can potentially distract/confuse the agents.
        """
...
class CommMARoomGrid(CommunicativeMultiAgentMiniGridEnv):
    """
    Environment with multiple agents, rooms, and random objects.
    This is meant to serve as a base class for other environments.
    """
